...
type StockDelta = Record<string, number>; // variantKey|drinkSizeKey → cantidad a sumar
...
export default function StockEntryModal({ products, onClose, onConfirm }: Props) {
  // deltas[productId][key] = cantidad a sumar
  ...
  // Total de unidades a ingresar (para el botón)
  ...
  const handleConfirm = () => {
    const result: Array<{ productId: number; key: string; delta: number }> = [];
    for (const [pidStr, keyMap] of Object.entries(deltas)) {
      ...
      for (const [key, delta] of Object.entries(keyMap)) {
        if (delta > 0) result.push({ productId, key, delta });
      }
    }

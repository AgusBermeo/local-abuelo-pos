...
/** Returns all variant keys for a food product: e.g. ["grande-carne","grande-pollo",...] */
function getFoodVariantKeys(product: Product): Array<{ variantKey: string; label: string }> {
...
function DrinkSizeEditor({
  ...
  return (
    <div className="flex flex-col gap-3">
        ...
        return (
          <div key={row.id} className="grid grid-cols-[1fr_auto_auto_2rem] gap-2 items-start">
            <div className="flex flex-col gap-1">
              ...
              />
              {errors[`ds_label_${row.id}`] && <p className="text-red-400 text-[10px]">{errors[`ds_label_${row.id}`]}</p>}
            </div>
            <div className="flex flex-col gap-1 w-24">
              ...
              {errors[`ds_price_${row.id}`] && <p className="text-red-400 text-[10px]">{errors[`ds_price_${row.id}`]}</p>}
            </div>
            <div className="flex flex-col gap-1 w-20">
              <input type="number" min={0} step={1} value={row.stock}
                onChange={(e) => update(row.id, "stock", e.target.value)}
                placeholder="0"
                className={`${inputClass} w-full text-center tabular-nums ${hasStock ? stockBorderFocus(stockNum) : ""} ${hasStock ? stockColor(stockNum) : ""}`}
              />
            </div>
            <button
              onClick={() => { if (rows.length > 1) onChange(rows.filter((r) => r.id !== row.id)); }}
      ...
      <button
        onClick={() => onChange([...rows, { id: `ds_${Date.now()}_${Math.random()}`, label: "", price: "", stock: "0" }])}
        className="self-start text-[10px] uppercase tracking-widest font-bold text-amber-600 hover:text-amber-400 border border-amber-800 hover:border-amber-600 rounded-lg px-3 py-1.5 transition-colors cursor-pointer"
      >
...
// ── VariantStockEditor ────────────────────────────────────────────────────────
/**
 * Visual table for editing stock per food variant.
 * variantKeys: all combinations (e.g. ["grande-carne","normal-carne",...])
 * stockMap: { "grande-carne": "20", ... }
 */
function VariantStockEditor({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {variantKeys.map(({ variantKey, label }) => {
        ...
        return (
          <div key={variantKey} className="grid grid-cols-[1fr_auto] gap-2 items-center">
            <span className={`text-xs font-semibold ${hasVal && num > 0 ? "text-amber-200" : "text-amber-700"}`}>
              {label}
            </span>
            <div className="w-28">
              <StepperInput
      ...
      <p className="text-[10px] text-amber-800">
        Stock en 0 = sin seguimiento. Mayor a 0 activa el control de inventario para esa variante.
      </p>
    </div>
  );
}
...
/** Compute variant keys from current form state */
...
function productToForm(product: Product): ProductForm {
  if (product.category === "Comida") {
    ...
    // Build variant stock map as strings
    ...
    return {
      ...
      tierRows, sizeEnabled, variantStockMap,
      drinkSizeRows: [defaultDrinkSizeRow()],
    };
  }
      ...
      {form.category === "Comida" && (
        <>
          ...
          <div className="flex flex-col gap-3 bg-amber-900/30 border border-amber-800 rounded-lg p-3">
            <p className="text-[10px] uppercase tracking-widest text-yellow-700">📦 Stock por variante</p>
            <VariantStockEditor
...
function formToProduct(form: ProductForm, id: number): Product {
  if (form.category === "Comida") {
    ...
    // Build variantStock from string map
                        ...
                        <div className="mt-2 pt-2 border-t border-amber-800/60 flex flex-col gap-1.5">
                          ...
                          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                            {variantKeys.map(({ variantKey, label }) => {
                              ...
                              const tracked = s > 0;
                              return (
                                <div key={variantKey} className="flex items-center justify-between gap-2 min-w-0">
                                  <span className="text-[10px] text-amber-300 truncate">{label}</span>
                                  {tracked ? (
                                    <span className={`text-[10px] font-bold tabular-nums shrink-0 ${stockColor(s)}`}>{s}</span>
                                  ) : (
